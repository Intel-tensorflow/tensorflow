...
def if_mkldnn_v3(if_true, if_false = []):
    ...
    """
    return select({
        "@org_tensorflow//third_party/mkl_dnn:build_with_mkldnn_v3": if_true,
        "//conditions:default": if_false,
    })

def if_mkldnn_v2(if_true, if_false = []):
    """Returns `if_true` if oneDNN v2.x is used.

    Shorthand for select()'ing on whether we're building with
    oneDNN v2.x open source library only, without depending on MKL binary form.

    Returns a select statement which evaluates to if_true if we're building
    with oneDNN v2.x open source library only. Otherwise, the select statement
    evaluates to if_false.

    """
    return select({
        "@org_tensorflow//third_party/mkl_dnn:build_with_mkldnn_v2": if_true,
        "//conditions:default": if_false,
    })

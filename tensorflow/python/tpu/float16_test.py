...
class Float16ScopeTest(test.TestCase):
  ...
  def testVariableName(self):
    ...
    with g.as_default():
      ...
      d = variables.Variable(4.4, name='var_d')
      with g.name_scope('scope1'):
        with float16.float16_scope('fp16'):
          a = math_ops.cast(a, dtypes.float16)
          b = math_ops.cast(b, dtypes.float16)
          c = math_ops.add(a, b, name='addition')
        with float16.float16_scope():
          d = math_ops.cast(d, dtypes.float16)
    ...
    self.assertIn('scope1/fp16/addition', ops_name)
    self.assertIn('scope1/fp16/Cast', ops_name)
  ...
  def testRequestedDType(self):
    ...
    """
    with float16.float16_scope() as scope:
      v1 = variable_scope.get_variable("v1", [])
      self.assertEqual(v1.dtype.base_dtype, dtypes.float32)
      v2 = variable_scope.get_variable("v2", [], dtype=dtypes.float16)
      self.assertEqual(v2.dtype.base_dtype, dtypes.float16)

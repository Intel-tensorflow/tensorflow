...
typename std::enable_if<std::is_same<Prim, dnnl::augru_forward>::value,
	                typename dnnl::augru_forward::primitive_desc*>::type
CreatePrimDesc(MklGRUParams<T> &params, dnnl::engine engine,
  ...
  return new dnnl::augru_forward::primitive_desc(engine, prop_k, direction,
                          *params.src_layer.desc, *params.src_iter.desc,
			  *params.attention.desc,
...
typename std::enable_if<std::is_same<Prim, dnnl::gru_forward>::value,
	                typename dnnl::gru_forward::primitive_desc*>::type
CreatePrimDesc(MklGRUParams<T> &params, dnnl::engine engine,
               dnnl::prop_kind prop_k, dnnl::rnn_direction direction) {
  params.show("GRUFwd");
  return new dnnl::gru_forward::primitive_desc(engine, prop_k, direction,
		          *params.src_layer.desc, *params.src_iter.desc,
			  *params.weights_layer.desc,
...
class GRUPrimitive {
 public:
  ...
  static std::string GetPrimitivekey() {
    ...
    if (std::is_same<primitiveDesc, dnnl::lbr_gru_forward::primitive_desc>()) {
      prefix = "lbr";
    }

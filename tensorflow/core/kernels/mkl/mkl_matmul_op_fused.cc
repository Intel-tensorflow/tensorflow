    ...
    for (const auto& post_op_info : post_op_info_list_) {
      ...
      switch (post_op_kind) {
        case PostOpKind::kOutputScale: {
          ...
          std::vector<float> wei_scale(num_output_channels);
          float min_min_weight = min_weight[0];
          float max_max_weight = max_weight[0];
          for (size_t i = 0; i < num_output_channels; ++i) {
            float range_weight =
                std::max(std::abs(min_weight[i]), std::abs(max_weight[i]));
            wei_scale[i] = range_weight / max_int8_weight;
            if (min_weight[i] < min_min_weight) min_min_weight = min_weight[i];
            if (max_weight[i] > max_max_weight) max_max_weight = max_weight[i];
          }
          ...
          params.post_op_params.push_back(
              {"wei_scale", wei_scale, wei_partial_key.GetKey()});
          const float single_wei_scale =
              std::max(std::abs(min_min_weight), std::abs(max_max_weight)) /
              max_int8_weight;
          const float dst_scale = std::is_same<Toutput, qint32>::value
                                      ? src_scale * single_wei_scale
                                      : 1.0;
          FactoryKeyCreator dst_partial_key;
          dst_partial_key.AddAsKey<float>(dst_scale);
          params.post_op_params.push_back(
              {"dst_scale", {dst_scale}, dst_partial_key.GetKey()});
        } break;
    ...
    if (std::is_same<Tbias, float>::value && input_quant_mode_ == "SCALED") {
      ...
      if (!is_cached_bias_valid) {
        ...
        void* scaled_bias_buf =
            static_cast<void*>(temp_scaled_bias_tensor->flat<float>().data());
        ...
        if (input_quant_mode_ == "MIN_FIRST") {
          ...
          float* adjusted_bias = (float*)scaled_bias_buf;
        ...
        *bias_data = temp_scaled_bias_tensor->flat<float>().data();
        ...
        if (is_bias_cache_empty) {
          ...
          this->CacheBias(ctx, *temp_scaled_bias_tensor, min_input, max_input,
                          &saved_min_input_, &saved_max_input_);
        }
      }
    }
  }
